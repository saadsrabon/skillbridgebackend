...
import { createBooking, updateBookingStatus, getBookingById, getStudentBookings, getTutorBookings, cancelBooking, getBookingsInDateRange, createReview, } from './booking.service';
/**
 ...
 */
export const getStudentBookingsController = async (req, res) => {
    try {
        ...
        const result = await getStudentBookings(studentId, status);
        if (result.success) {
            res.status(200).json(result);
        }
        else {
 ...
 * Query params: status (optional)
 */
export const getTutorBookingsController = async (req, res) => {
    try {
        ...
        const result = await getTutorBookings(tutorId, status);
        if (result.success) {
            res.status(200).json(result);
        }
        else {
 ...
 */
export const getBookingsInDateRangeController = async (req, res) => {
    try {
        ...
        const result = await getBookingsInDateRange(start, end, tutorId, status);
        if (result.success) {
            res.status(200).json(result);
        }
        else {
