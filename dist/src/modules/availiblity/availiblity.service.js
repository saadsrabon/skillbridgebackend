 ...
 */
const checkSlotConflict = async (tutorId, startTime, endTime, excludeSlotId) => {
    const where = {
        tutorId,
        OR: [
            {
 ...
 */
export const getTutorAvailabilitySlots = async (tutorId, includeBooked = true) => {
    try {
        // Verify tutor exists
        const tutorExists = await prisma.tutorProfile.findUnique({
 ...
 */
export const getAvailableSlotsInRange = async (tutorId, startDate, endDate) => {
    try {
        // Verify tutor exists
        const tutorExists = await prisma.tutorProfile.findUnique({
            where: { id: tutorId },
...
//# sourceMappingURL=availiblity.service.js.map